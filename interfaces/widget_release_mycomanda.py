...
class WidgetReleaseMyComanda(BaseWindow):
    def __init__(self, parent=None):
        super(WidgetReleaseMyComanda, self).__init__(parent)
    ...
    def create_all_labels(self):
        ...
        self.label_version_mycomanda = self.create_label(
            text="Versão do MyComanda: "
        )
        self.horizontal_layout_release.addWidget(self.label_version_mycomanda)
    ...
    def create_all_line_edits(self):
        ...
        self.line_edit_version_mycomanda_release = self.create_line_edit(
            placeholder="versão do MyComanda",
            set_text=self.latest_version_handler.latest_release_version_text_mycomanda()
        )
        self.horizontal_layout_release.addWidget(self.line_edit_version_mycomanda_release)
        
        self.process_input(self.line_edit_version_mycomanda_release)
        self.process_input(self.line_edit_mycommerce_version)
        
        # passa para o proximo line_edit quando o usuario pressionar enter
        self.line_edit_return_pressed(self.line_edit_mycommerce_version, self.line_edit_version_mycomanda_release)
        self.line_edit_return_pressed(self.line_edit_version_mycomanda_release, self.line_edit_mycommerce_version)     

if __name__ == "__main__":
    import sys
    from PySide6.QtWidgets import QApplication
    app = QApplication(sys.argv)
    window = WidgetReleaseMyComanda()
    window.show()
    app.exec()